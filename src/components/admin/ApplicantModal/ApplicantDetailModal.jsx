...
import { DetailItem, EditInput, EditSelect } from "./SharedUI";
import DocumentPreview from "./DocumentPreview";
import { AdminReviewPanel, PromotePanel } from "./ActionPanels";
...
export default function ApplicantDetailModal({
  ...
  useEffect(() => setIsZoomed(false), [previewImage]);
  ...
  const startEditing = () => {
    setEditData({
      ...
      marriage_date:
        ...
        String(selectedApplicant.marriageDate).toLowerCase() !== "invalid date"
          ? selectedApplicant.marriageDate
          : "",
  ...
  const handleSaveEdit = async () => {
    ...
    if (
      ...
      payloadToSave.marriage_date === "" ||
      String(payloadToSave.marriage_date).toLowerCase() === "invalid date"
    )
  ...
  const renderViewDetails = () => (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 print:gap-y-6">
        ...
        <DetailItem
          label="From Raipur?"
          value={selectedApplicant.isFromRaipur ? "Yes" : "No"}
        />
          ...
          <div>
            ...
            <div className="h-32 bg-gray-50 rounded-lg border border-gray-300 overflow-hidden relative group shadow-inner">
              {getFileUrl("AADHAR_FRONT") ? (
                <button
                  type="button"
                  onClick={() => setPreviewImage(getFileUrl("AADHAR_FRONT"))}
          ...
          <div>
            ...
            <div className="h-32 bg-gray-50 rounded-lg border border-gray-300 overflow-hidden relative group shadow-inner">
              {getFileUrl("AADHAR_BACK") ? (
                <button
                  type="button"
                  onClick={() => setPreviewImage(getFileUrl("AADHAR_BACK"))}
                  className="w-full h-full block cursor-pointer"
                >
                  <img
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/80 backdrop-blur-sm transition-opacity print:static print:bg-transparent print:p-0">
      ...
      >
        ...
        <div
          className={`flex-1 overflow-y-auto flex flex-col ${previewImage ? "lg:border-r-2 lg:border-gray-200" : ""} print:block print:overflow-visible`}
        >
          <div className="sticky top-0 bg-white/95 backdrop-blur border-b border-gray-100 px-6 py-4 flex justify-between items-center z-10 print:hidden shadow-sm">
            <div>
              ...
              <p className="text-sm text-indigo-600 font-semibold mt-1">
                Ref ID: {selectedApplicant.id.substring(0, 8).toUpperCase()}
              </p>
            </div>
              ...
              {!isEditing &&
                selectedApplicant.status === "PENDING_ADMIN_REVIEW" && (
                  <button
                    ...
                    className="flex items-center space-x-2 bg-orange-50 text-orange-700 hover:bg-orange-600 hover:text-white px-4 py-2 rounded-lg font-bold transition-colors border border-orange-200"
                  >
                    <svg
                      className="w-5 h-5"
                      fill="none"
          ...
          <div className="hidden print:block text-center border-b-2 border-gray-800 pb-4 mt-6 mx-8">
            <h1 className="text-3xl font-extrabold text-gray-900">
              महाराष्ट्र मंडळ, रायपूर, छत्तीसगढ़
            </h1>
            <h2 className="text-xl font-bold mt-2 text-gray-800">
              सभासद आवेदन पत्र (आजीवन)
            </h2>
          </div>
          ...
          <div className="p-6 pt-0 print:pt-6">
              ...
              <div className="flex-1 space-y-3">
                ...
                {selectedApplicant.proposer && (
                  <div className="text-sm text-gray-800 text-lg print:text-base">
                    <span className="font-semibold">अनुमोदक (Proposer):</span>{" "}
                ...
                {selectedApplicant.registrationNumber && (
                  <p className="text-lg font-bold text-black mt-2">
                    पंजीकरण संख्या: {selectedApplicant.registrationNumber}
                  </p>
                )}
              </div>
            ...
            {isEditing ? renderEditForm() : renderViewDetails()}
              ...
              <div className="text-center ml-auto">
                <div className="w-48 h-16 bg-transparent flex items-end justify-center mb-2 overflow-hidden print:border-b print:border-black group relative">
                  {getFileUrl("SIGNATURE") ? (
                    <button
                      type="button"
                      onClick={() => setPreviewImage(getFileUrl("SIGNATURE"))}
                      className="w-full h-full block cursor-pointer"
                    >
                      <img
...
const applicantShape = PropTypes.shape({
  ...
  isFromRaipur: PropTypes.bool,
  region: PropTypes.string,
  status: PropTypes.string,
  registrationNumber: PropTypes.string,
  proposer: PropTypes.shape({ name: PropTypes.string }),
  files: PropTypes.arrayOf(
    PropTypes.shape({ fileType: PropTypes.string, minioUrl: PropTypes.string }),
  ),
});

ApplicantDetailModal.propTypes = {
  selectedApplicant: applicantShape.isRequired,
  setSelectedApplicant: PropTypes.func.isRequired,
  loadingDetails: PropTypes.bool.isRequired,
  loadApplicants: PropTypes.func.isRequired,
